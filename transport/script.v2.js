...
d3.xml("data/map.svg", "image/svg+xml", function(error, xml) {
  ...
  .await(function(error, data) {
    ...
    for(item in data.links) {
      var link = d3.select("#" + item);
      if(link.attr("d1") != null) {
        data.links[item].d0 = link.attr("d");
        data.links[item].d1 = link.attr("d1");
        data.links[item].d2 = link.attr("d2");
        link.attr("d1", null);
        link.attr("d2", null);
      }
    }
    function Pin() {
      ...
      basemap.on("click", function (event) {
        ...
        if(!pinned) {
          ...
          for(item in data.links) {
            data.links[item].status ? data.links[item].pinned = true : data.links[item].pinned = false
          }
          for(item in data.routes) {
            data.routes[item].status ? data.routes[item].pinned = true : data.routes[item].pinned = false
          }
          for(item in data.nodes) {
            data.nodes[item].status ? data.nodes[item].pinned = true : data.nodes[item].pinned = false
          }
          // for(item in data.nodes) {
      ...
      this.highlightLinks = function (a) {
        var pinnedRoutes = new Array;
        var nonPinnedRoutes = new Array;
        if(false) {
          popup.selectAll(".route_num").remove();
          for(item in data.routes) {
            var b = checkStatus(data.routes[item].nodes, a);
            //console.log(data.routes[item].pinned);
            b && data.routes[item].pinned ? pinnedRoutes.push(item) : null
            b && !data.routes[item].pinned ? nonPinnedRoutes.push(item) : null
            if(b) {
              //activeRoutes.push(item);
              d3.select("#" + data.routes[item].type).append("div")
                .attr("class", "route_num")
                .attr("id", item)
                .style("background-color", function () {
                  return data.routes[item].pinned ? colors.orange.less : colors.blue.less
                })
                .style("color", "white")
                .text(item.split("_")[1]);
            } else {
              if(data.routes[item].pinned) {
                d3.select("#" + data.routes[item].type).append("div")
                  .attr("class", "route_num")
                  .attr("id", item)
          ...
          var activeRoutes = pinnedRoutes.concat(nonPinnedRoutes);
        ...
        for(item in data.links) {
          if(!pinned) {
            ...
            var b = data.links[item].pinned;
            var b0 = checkStatus(data.links[item].routes, pinnedRoutes);
            var b1 = checkStatus(data.links[item].routes, nonPinnedRoutes);
            //var b = b0 || b1
            //console.log(data.links[item].phase == 2);
            if(data.links[item].pinned && b1 && !data.links[item].double) {
              var right = d3.select("#" + item)
              var left, center;
              if(d3.select("#" + item + "_left").node() == null) {
                //right = d3.select(link.node().parentNode.insertBefore(d3.select(link.node().cloneNode(true)).node(), link.node()));
                left = d3.select(right.node().parentNode.insertBefore(d3.select(right.node().cloneNode(true)).node(), right.node()));
                left.attr("id", item + "_left")
                  .style("stroke", colors.blue.main)
                center = d3.select(right.node().parentNode.appendChild(right.node().cloneNode(true)));
                center.attr("id", item + "_center")
                  .attr("d", data.links[item].d2)
                  .style("stroke", colors.grey["1000"])
                  .style("stroke-width", 0);
              } else {
                left = d3.select("#" + item + "_left");
                center = d3.select("#" + item + "_center");
              }

              right.transition().ease("cubic-out").duration(500)
                .attr("d", data.links[item].d1)

              left.transition().ease("cubic-out").duration(500)
                .attr("d", data.links[item].d2)

              center.transition().ease("cubic-out").duration(500)
                .attr("d", data.links[item].d0)
                .style("stroke-width", 3);

              data.links[item].double = true;
            }
            if(data.links[item].pinned && !b1 && data.links[item].double) {
              var right = d3.select("#" + item);
              var left = d3.select("#" + item + "_left");
              var center = d3.select("#" + item + "_center");
              right.transition().ease("cubic-out").duration(500)
                .attr("d", data.links[item].d0)
              ...
              data.links[item].double = false;
            }
            if(!data.links[item].pinned && b1) {
                d3.select("#" + item).transition().ease("cubic-out").duration(500)
                  .style("stroke", colors.blue.main)
            }
            if(!data.links[item].pinned && !b1) {
            ...
            // if(data.links[item].pinned && !b0) {
            //   d3.select("#" + item).transition().ease("cubic-out").duration(500)
            //     .attr("d", function () {  return data.links[item].double ? data.links[item].d1 : data.links[item].d0  })
            //     .style("stroke", colors.orange.dark)
            //     .style("stroke-width", 3)
            // }
            // if(data.links[item].pinned && b0) {
            //   d3.select("#" + item).transition().ease("cubic-out").duration(500)
        ...
        for(item in data.nodes) {
          ...
          var b = checkStatus(data.nodes[item].routes, activeRoutes);
          var b0 = checkStatus(data.nodes[item].routes, pinnedRoutes);
          var b1 = checkStatus(data.nodes[item].routes, nonPinnedRoutes);
          if(!pinned) {
            ...
            if(b0 && b1 && data.nodes[item].phase != 0) {
              d3.select("#area_" + item).transition().ease("cubic-out").duration(500)
                .style("fill", "white")
                .style("opacity", .5);

              data.nodes[item].phase = 0;
            }
            if(!b0 && b1 && data.nodes[item].phase != 1) {
              d3.select("#area_" + item).transition().ease("cubic-out").duration(500)
                .style("fill", colors.blue.main)
                .style("opacity", .66);

              data.nodes[item].phase = 1;
            }
            if(b0 && !b1 && data.nodes[item].phase != 2) {
              d3.select("#area_" + item).transition().ease("cubic-out").duration(500)
                .style("fill", colors.orange.main)
                .style("opacity", .66);

              data.nodes[item].phase = 2;
            }
            if(!b0 && !b1 && data.nodes[item].phase != 3) {
              d3.select("#area_" + item).transition().ease("cubic-out").duration(500)
                .style("fill", function () {
                  return data.nodes[item].pinned ? colors.orange.main : colors.road.l
                })
                .style("opacity", .66);

              data.nodes[item].phase = 3;
            }
          }
          data.nodes[item].onRoute = b;
          var b = data.nodes[item].h < radius * r;
